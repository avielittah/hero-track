...
export function ApplicationSkillsStage() {
  ...
  const canAccessStage4 = stage3Completed || adminBypass;
  ...
  const handleUnitStart = (unitId: string) => {
    // Check if user can access Stage 4
    if (!canAccessStage4) {
  ...
  return <div className="min-h-screen bg-background relative">
      ...
      {/* Preview Banner */}
      {isPreviewMode && <CompletionBanner stageName="Application Skills" />}
      
      <div className="container mx-auto px-4 py-8 max-w-5xl" dir="ltr">
        {/* Stage Intro */}
        <StageIntro title="Application Skills — Advanced Practice" description="Build advanced skills through hands-on application of core engineering tools and methodologies." estimatedTime="~40–60 minutes total." xpTarget={30} icon={<Wrench className="h-8 w-8" />} />
        ...
        y: 0
            ...
            <StageSummary units={units} earnedXP={earnedXP} totalXP={30} canComplete={canComplete || adminBypass} onComplete={handleStageComplete} nextStageName="Mid-Journey Survey" />
          </motion.div>
