...
export function TechnicalOrientationStage() {
  ...
  return <div className="min-h-screen bg-background relative">
      ...
      {/* Preview Banner */}
      {isPreviewMode && <CompletionBanner stageName="Technical Orientation" />}
      
      <div className="container mx-auto px-4 py-8 max-w-5xl" dir="ltr">
        {/* Stage Intro */}
        <StageIntro title={stage3Content.title} description={stage3Content.intro} estimatedTime={stage3Content.estTime} xpTarget={stage3Content.xpTotalTarget} icon={<Wrench className="h-8 w-8" />} />
        ...
        <div className="space-y-8 mb-8">
          ...
          <ToolCard id="vlc" title={stage3Content.units.vlc.title} description={stage3Content.units.vlc.objective} estimatedTime={stage3Content.units.vlc.estimatedTime} icon={<Play className="h-6 w-6" />} isCompleted={vlcSubmitted} videoUrl={stage3Content.units.vlc.videoUrl} toolLink={stage3Content.units.vlc.toolLink} onStart={() => handleUnitStart('vlc')} isDisabled={isPreviewMode && !vlcSubmitted} />
        </div>

        {/* Mini Quiz */}
        {(drawioSubmitted || vlcSubmitted) && <motion.div initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        ...
        {/* Stage Lock Modal */}
        <AnimatePresence>
          {showLockModal && <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} exit={{
          opacity: 0
        }} className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowLockModal(false)}>
              <motion.div initial={{
            scale: 0.95,
            opacity: 0
          }} animate={{
            ...
            scale: 0.95,
            opacity: 0
          }} className="bg-card rounded-2xl border-2 p-8 max-w-md w-full text-center space-y-6" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex gap-3">
                  <Button variant="outline" onClick={() => setShowLockModal(false)} className="flex-1">
                    Stay Here
                  </Button>
                  <Button onClick={() => {
        ...
        delay: 0.5
            ...
            {canComplete && !stageFeedback && <StageFeedback onSubmit={feedback => {
        ...
        {!canComplete && !isPreviewMode && <StageSummary units={units} earnedXP={earnedXP} totalXP={stage3Content.xpTotalTarget} canComplete={canComplete || adminBypass} onComplete={handleStageComplete} nextStageName="Hands-On Practice" />}
        ...
        delay: 0.8
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <DidYouKnowBox title="🎯 Professional Impact" content="Engineers who document their work with clear diagrams are 40% more effective in team collaborations. The visual communication skills you've learned here will set you apart in your career!" xpReward={3} onRewardClaim={() => handleBonusXPClaim(3)} onClose={() => {}} />
              <DidYouKnowBox title="🔧 Real-World Usage" content="VLC's stream analysis features are used by broadcast engineers worldwide to debug live transmission issues. You now have the same professional-grade tools they use!" xpReward={3} onRewardClaim={() => handleBonusXPClaim(3)} onClose={() => {}} />
            </div>
            ...
            {/* Stage Feedback */}
            {!stageFeedback && <StageFeedback onSubmit={feedback => {
          setStageFeedback(feedback);
          toast({
            title: "Thanks for your feedback! 💙",
            description: "Your input helps us improve the learning experience."
